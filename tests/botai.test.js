...
describe('findGroupCandidates', () => {
  ...
  test('G7 Dois pares com dois coringas gera candidatos distintos (sem conflito)', () => {
    const hand = [card('K', '♠', 1), card('K', '♥', 1), card('Q', '♠', 1), card('Q', '♥', 1), wild(1), wild(2)];
    const cands = findGroupCandidates(hand, 'medium', true);
...
describe('findExtensionCandidates', () => {
  ...
  test('E7 Coringa na borda direita: carta original (posição do wild) é identificada', () => {
    ...
    assert.ok(
      cands.some((c) => c.cardIds.includes('8♥-1')),
      'Deve identificar 8♥ quando wild está na borda direita',
    );
  });
  ...
  test('E8 Coringa na borda esquerda: carta original é identificada', () => {
    ...
    const cands = findExtensionCandidates(hand, [meld], 'medium');
    assert.ok(
      cands.some((c) => c.cardIds.includes('5♥-1')),
      'Deve identificar 5♥ quando wild está na borda esquerda',
    );
  });
  ...
  test('E9 Coringa na borda: carta ALÉM do wild também é identificada', () => {
    ...
    const cands = findExtensionCandidates(hand, [meld], 'medium');
    assert.ok(
      cands.some((c) => c.cardIds.includes('9♥-1')),
...
describe('decideMeldActions', () => {
  ...
  test('M2 Jogo normal: grupos de ranks 5-10 NÃO são baixados', () => {
    const hand = [
      card('7', '♠', 1),
      card('7', '♥', 1),
      card('7', '♦', 1), // grupo 7s (lowPriority)
      card('3', '♠', 1),
      card('3', '♥', 1),
    ];
    const g = mockGame({ hand, hasFirstMeld: true });
    const actions = decideMeldActions(g, 0, 'hard');
    const melds7 = actions.filter((a) => a.type === 'new' && a.cards.some((id) => id.startsWith('7')));
  ...
  test('M3 Jogo normal: coringas NÃO são usados em novos grupos mistos', () => {
    ...
    const wildGroups = actions.filter(
      (a) => a.type === 'new' && a.cards.some((id) => id.startsWith('2')) && a.cards.some((id) => id.startsWith('K')),
    );
